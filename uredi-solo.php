...
if (isset($_GET['edit'])) {
  $id = $_GET['edit'];
  $update = true;
  $record = mysqli_query($konekcija, "SELECT * FROM proizvodi WHERE id=$id");
...
if (isset($_POST['azuriraj'])) {
  ...
  mysqli_query($konekcija, "UPDATE proizvodi SET ime='$ime', vrsta='$vrsta', brend='$brend', poslovnica='$poslovnica', stanje='$stanje', cijena='$cijena' WHERE id=$id");
  header('location: uredi.php');
}
...
if (isset($_SESSION['id']) && isset($_SESSION['korisnicko_ime'])) {
 ...
 <div class="w3-top">
   <div class="w3-bar w3-pink w3-card w3-left-align w3-large">
     ...
     <a href="uredi.php" class="w3-bar-item w3-button w3-hide-small w3-padding-large w3-white">Uredi proizvod</a>
...
<div class="w3-row-padding w3-padding-64 w3-container w3-center">
  ...
  <h1>Uredi proizvod "<?php echo $ime; ?>"</h1>
  <form method="post" action="uredi-solo.php" >
    <input type="hidden" name="id" value="<?php echo $id; ?>" class="w3-input w3-border">
    <div class="input-group">
      <label>Ime</label>
      <input type="text" name="ime" value="<?php echo $ime ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <label>Vrsta</label>
      <input type="text" name="vrsta" value="<?php echo $vrsta ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <label>Brend</label>
      <input type="text" name="brend" value="<?php echo $brend ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <label>Poslovnica</label>
      <input type="text" name="poslovnica" value="<?php echo $poslovnica ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <label>Stanje</label>
      <input type="text" name="stanje" value="<?php echo $stanje ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <label>Cijena</label>
      <input type="text" name="cijena" value="<?php echo $cijena ?>" class="w3-input w3-border" required>
    </div>
    <div class="input-group">
      <button class="w3-button w3-green" type="submit" style="margin-top: 20px" name="azuriraj" >Ažuriraj</button>
    </div>
  </form>
  </div>
